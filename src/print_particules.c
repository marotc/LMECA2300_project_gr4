...
// 	float rmax = 100.0*sqrtf(2.0f);
	...
	for (int i = 0; i < N; i++) {
        xy fs;
        fs.x = -particles[i]->param->sigma * particles[i]->normal->x * particles[i]->kappa;
        fs.y = -particles[i]->param->sigma * particles[i]->normal->y * particles[i]->kappa;
...
// 		  colormap_uni_color_2(&data[i][4]);
		...
		color_temp(p,&data[i][4]);


		data[i][7] = 1.0; // transparency
	}
...
Animation* Animation_new(int N, double timeout,Grid* grid,double scale)
{
	...
	bov_points_set_width(particles, 0.005); //To change to 0.001 for dam_break
    ...
    animation->contiView = newContinuousViewer();
...
{
    freeContinuousViewer(animation->contiView);
...
//    fflush(stdout);
	...
	char screenshot_name[64] = "test/test_";
	char int_string[32];
...
// 	double fs_norm = 1.0;//norm(fs);
	...
	color[0] = 2*squared(fs->x/max_norm) + 2*squared(fs->y/max_norm);
	color[1] = 0.0;
...
void color_temp(Particle *p,float color[3]){
    float v = (p->temp - p->temp_min) / (p->temp_max - p->temp_min);
